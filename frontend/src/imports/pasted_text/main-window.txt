...
class MainWindow(QMainWindow):
    ...
    def _toolbar_button(self, kind: str, label: str, handler, tooltip: str, primary: bool = False) -> QPushButton:
        """Create an icon-only, square toolbar button with a rich hover tooltip."""
        button = QPushButton()
    ...
    def _build_ui(self) -> None:
        ...
        # ---------------------------------------------------------------
        # App bar: brand mark, primary navigation, and appearance control.
        # ---------------------------------------------------------------
        app_bar = QFrame()
        app_bar.setObjectName("appBar")
        app_bar_layout = QHBoxLayout(app_bar)
        app_bar_layout.setContentsMargins(20, 12, 20, 12)
        app_bar_layout.setSpacing(10)

        brand_mark = QLabel("◆")
        brand_mark.setObjectName("brandMark")
        brand_mark.setFixedWidth(22)
        app_bar_layout.addWidget(brand_mark)

        brand_title = QLabel("Visual Git Workspace")
        brand_title.setObjectName("brandTitle")
        app_bar_layout.addWidget(brand_title)
        app_bar_layout.addSpacing(18)

        open_button = QPushButton(self.style().standardIcon(QStyle.StandardPixmap.SP_DialogOpenButton), "Open Repository")
        ...
        # ---------------------------------------------------------------
        # Body: everything below the app bar sits on a padded canvas.
        # ---------------------------------------------------------------
        body = QWidget()
        ...
        # ---------------------------------------------------------------
        # Action rail: filesystem operations and Git operations as two
        # clearly labeled, visually separated tool groups.
        # ---------------------------------------------------------------
        actions_card = QFrame()
        actions_card.setObjectName("toolRail")
        actions_outer = QVBoxLayout(actions_card)
        actions_outer.setContentsMargins(14, 10, 14, 10)
        actions = QHBoxLayout()
        actions.setSpacing(8)

        filesystem_label = QLabel("FILESYSTEM")
        filesystem_label.setObjectName("sectionTag")
        actions.addWidget(filesystem_label)
        for kind, label, handler, tooltip in (
            ...
            actions.addWidget(self._toolbar_button(kind, label, handler, tooltip))
        ...
        for kind, label, handler, tooltip, emphasis in (
            ...
            actions.addWidget(self._toolbar_button(kind, label, handler, tooltip, primary=emphasis))
        actions.addStretch()
        actions_outer.addLayout(actions)
        layout.addWidget(actions_card)

        # Cloud controls remain in the codebase but are intentionally not presented while remote work is frozen.
        ...
        # ---------------------------------------------------------------
        # Workspace: resizable sidebar + explorer, all panels stretch
        # smoothly as the window is resized.
        # ---------------------------------------------------------------
        ...
        breadcrumb_card = QFrame()
        breadcrumb_card.setObjectName("breadcrumbCard")
        breadcrumb_card_layout = QHBoxLayout(breadcrumb_card)
        breadcrumb_card_layout.setContentsMargins(4, 4, 4, 4)
        self.breadcrumb_widget = QWidget()
        self.breadcrumb_layout = QHBoxLayout(self.breadcrumb_widget)
        self.breadcrumb_layout.setContentsMargins(8, 4, 8, 4)
        self.breadcrumb_layout.setSpacing(4)
        breadcrumb_card_layout.addWidget(self.breadcrumb_widget)
        content_layout.addWidget(breadcrumb_card)

        self.tree = FileTreeWidget()
        self.tree.setObjectName("explorerTree")
        self.tree.setFrameShape(QFrame.Shape.NoFrame)
        self.tree.setHeaderLabels(["Name", "Git Status", "Type"])
        ...
        tree_card = QFrame()
        tree_card.setObjectName("explorerCard")
        tree_card_layout = QVBoxLayout(tree_card)
        tree_card_layout.setContentsMargins(1, 1, 1, 1)
        ...
        workflow_layout.addWidget(self.workflow_happening_label)
        workflow_layout.addWidget(self.workflow_message_label)
        workflow_layout.addSpacing(6)
        ...
        explorer_splitter.setHandleWidth(10)
        explorer_splitter.addWidget(tree_card)
        ...
        explorer_splitter.setStretchFactor(0, 3)
        ...
        workspace_splitter.setSizes([220, 900])
        layout.addWidget(workspace_splitter, 1)

        for panel, blur, alpha in (
            (sidebar, 18, 22),
            (tree_card, 18, 22),
            (workflow_widget, 20, 26),
            (changes_group, 20, 26),
            (actions_card, 16, 18),
            (breadcrumb_card, 12, 16),
        ):
            self._elevate(panel, blur=blur, alpha=alpha)
    ...
    def _set_empty_state(self) -> None:
        self.repository_label.setText("No repository open. Choose a local Git repository to begin.")
    ...
    def _populate_item(self, parent_item: QTreeWidgetItem, entries: list[Path], statuses: dict[Path, object]) -> None:
        for entry in entries:
            ...
            child = QTreeWidgetItem([entry.name, status.label if status else "Unchanged", kind])
            ...
            if entry.is_dir() and not entry.is_symlink():
                try:
                    ...
                    child.setText(1, "Folder (unreadable)")
    ...
    def _update_changes_panel(self, statuses: dict[Path, object]) -> None:
        ...
        working_tree: list[tuple[str, Path]] = []
        for relative_path, status in sorted(statuses.items(), key=lambda item: item[0].as_posix().casefold()):
            if status.index_code != " " and status.index_code != "?":
                staged.append((f"{status.symbol} {relative_path} ({status.label})", relative_path))
            if status.worktree_code != " " or status.worktree_code == "?":
                working_tree.append((f"{status.symbol} {relative_path} ({status.label})", relative_path))
        self.changes_list.addItem(f"Staged changes ({len(staged)})")
        for text, relative_path in staged:
            self._add_change_item(text, relative_path, True)
        self.changes_list.addItem(f"Working tree changes ({len(working_tree)})")
        for text, relative_path in working_tree:
            self._add_change_item(text, relative_path, False)
    ...
    def _update_navigation(self) -> None:
        if self.repository is None or self.current_directory is None:
            ...
            return
        self.locations_list.clear()
        for label, path in (("Repository root", self.repository.root), ("Current folder", self.current_directory)):
    ...
    def _update_status_bar(self, statuses: dict[Path, object]) -> None:
        ...
        for status in statuses.values():
            labels[status.label.lower()] = labels.get(status.label.lower(), 0) + 1
        summary = " • ".join(f"{count} {label}" for label, count in sorted(labels.items())) or "clean"
        branch = self.repository.branch or "detached HEAD"
        self.statusBar().showMessage(f"Git: {branch}  •  {summary}")

    def _apply_theme(self, theme: str) -> None:
        dark = theme == "Dark"

        # A restrained, professional slate + indigo palette. Indigo is used
        # sparingly (brand mark, primary actions, focus/selection) so it
        # reads as an accent rather than decoration.
        colors = {
            "window": "#0f1420" if dark else "#eef1f6",
            "canvas": "#141a28" if dark else "#eef1f6",
            "panel": "#1b2233" if dark else "#ffffff",
            "panel_alt": "#171d2c" if dark else "#fbfcfe",
            "text": "#e7ebf3" if dark else "#1a2130",
            "muted": "#8892a6" if dark else "#6b7486",
            "faint": "#5c6478" if dark else "#98a1b3",
            "border": "#2a3345" if dark else "#e0e4ec",
            "border_strong": "#38425a" if dark else "#cdd3e0",
            "accent": "#818cf8" if dark else "#4f46e5",
            "accent_hover": "#9aa4fb" if dark else "#4338ca",
            "accent_soft": "#242c47" if dark else "#eef0ff",
            "selection": "#2f3a63" if dark else "#e4e7ff",
            "selection_text": "#eef0ff" if dark else "#1e1b4b",
            "helper": "#1a2338" if dark else "#f7f8fd",
            "appbar": "#121826" if dark else "#ffffff",
            "success": "#34d399" if dark else "#16a34a",
        ...
        QApplication.instance().setStyleSheet(
            ...
            QWidget#canvas { background: %(canvas)s; }
            QWidget { color: %(text)s; font-family: "Segoe UI", "Inter", "Helvetica Neue", sans-serif; font-size: 13px; }

            /* ---- App bar ------------------------------------------------ */
            QFrame#appBar { background: %(appbar)s; border: 0; border-bottom: 1px solid %(border)s; }
            QLabel#brandMark { color: %(accent)s; font-size: 17px; }
            QLabel#brandTitle { color: %(text)s; font-size: 15px; font-weight: 600; }
            QLabel#mutedLabel { color: %(faint)s; font-size: 12px; }

            /* ---- Typography helpers -------------------------------------- */
            QLabel#sectionTag { color: %(faint)s; font-size: 11px; font-weight: 600; padding: 2px 0; }
            QLabel#repositorySummary { color: %(muted)s; font-size: 12px; }
            QLabel#workflowMessage { color: %(text)s; font-size: 13px; }
            ...
            QPushButton:disabled { color: %(faint)s; background: %(panel_alt)s; border-color: %(border)s; }
            ...
            QPushButton#primaryButton:hover { background: %(accent_hover)s; border-color: %(accent_hover)s; color: #ffffff; }
            QPushButton[toolIcon="true"] { padding: 4px; border-radius: 8px; }
            QPushButton[toolIcon="true"]:hover { background: %(accent_soft)s; }
            QPushButton#primaryButton[toolIcon="true"]:hover { background: %(accent_hover)s; }
            QPushButton:flat { border: none; padding: 3px 6px; background: transparent; }
            QPushButton:flat:hover { color: %(accent)s; background: %(accent_soft)s; border-radius: 4px; }
            ...
            /* ---- Panels / cards -------------------------------------------*/
            QFrame#toolRail, QFrame#breadcrumbCard, QFrame#sidebarPanel,
            QFrame#explorerCard, QFrame#workflowHelper, QFrame#changesPanel {
                background: %(panel)s;
                border: 1px solid %(border)s;
                border-radius: 10px;
            }
            QFrame#workflowHelper { background: %(helper)s; }
            QFrame#verticalDivider { color: %(border)s; }
            ...
            QTreeWidget#explorerTree, QListWidget {
                selection-background-color: %(selection)s;
            }
            QHeaderView::section {
                background: %(panel)s;
            ...
            QTreeWidget::item:hover, QListWidget::item:hover { background: %(accent_soft)s; }
            ...
            /* ---- Misc --------------------------------------------------------*/
            QLabel { color: %(text)s; }
            QStatusBar { background: %(appbar)s; color: %(muted)s; border-top: 1px solid %(border)s; font-size: 12px; }
            QMenuBar { background: %(appbar)s; border-bottom: 1px solid %(border)s; }
            QMenuBar::item:selected { background: %(accent_soft)s; border-radius: 4px; }
            QMenu { background: %(panel)s; border: 1px solid %(border)s; }
            QMenu::item:selected { background: %(selection)s; color: %(selection_text)s; }
            QScrollBar:vertical, QScrollBar:horizontal { background: transparent; margin: 2px; }
            QScrollBar::handle { background: %(border_strong)s; border-radius: 5px; min-height: 24px; min-width: 24px; }
        ...
        # Recolor every icon-only toolbar button to match the active theme.
        # Primary (accent-background) buttons get a white icon for contrast;
        # everything else uses the muted foreground color.
        icon_color = QColor(colors["muted"])
        primary_icon_color = QColor("#ffffff")
        for button, (kind, primary) in self._icon_buttons.items():
            button.setIcon(self._vector_icon(kind, primary_icon_color if primary else icon_color, size=20))

    def _repository_summary(self) -> str:
        assert self.repository is not None
        branch = self.repository.branch or "detached HEAD"
        return f"Repository: {self.repository.root.name}\nPath: {self.repository.root}\nGit: detected | Branch: {branch}"
